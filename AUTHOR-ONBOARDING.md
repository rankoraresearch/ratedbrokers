...
### Самый простой путь: UI в дашборде (рекомендуется)

Открой `https://api.ratedbrokers.com/api/admin/submissions/dashboard?key=$API_KEY` и в верхней части разверни блок **"👤 Invite a new author"**. Заполни форму — появится кнопка **"Copy invite URL"**. Всё.

Ниже формы автоматически показана таблица всех существующих авторов с кнопками **Rotate** (новый токен) и **Revoke/Activate**.

### Альтернатива: curl (для скриптов и автоматизации)
