    ...
    <div style={{fontFamily:"'DM Sans',system-ui,sans-serif",background:"#f8f9fb",color:"#1e293b"}}>
      ...
      <div style={{...cn,display:mob?"flex":"grid",flexDirection:"column",gridTemplateColumns:mob?"1fr":tab?"1fr 220px":"200px 1fr 260px",gap:mob?16:24,paddingTop:mob?20:28,paddingBottom:mob?40:64}}>
        ...
        {!mob&&!tab&&<aside style={{position:"sticky",top:70,alignSelf:"start"}}>
          <div style={{fontSize:10,fontWeight:600,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:10}}>{t("review.contents")}</div>
          {TOC.map((tocItem,i)=>{const id=tocItem.toLowerCase().replace(/[^a-z0-9]+/g,'-');return <a key={i} href={`#${id}`} style={{display:"block",fontSize:12,color:"#64748b",textDecoration:"none",padding:"5px 10px",borderLeft:"2px solid #e2e8f0",marginBottom:1,lineHeight:1.4}}>{tocItem}</a>;})}
        </aside>}
        ...
        <main>
          ...
          <Card>
            {scores.map((s,i)=>{
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                {isTp ? <a href={getTrustpilotUrl(slug)} target="_blank" rel="noopener noreferrer" style={{fontSize:14,fontWeight:600,color:"#1e293b",textDecoration:"none",display:"flex",alignItems:"center",gap:4}}>{s.name}<svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M2.5 9.5L9.5 2.5M9.5 2.5H4M9.5 2.5V8" stroke="#00B67A" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg></a> : <span style={{fontSize:14,fontWeight:600,color:"#1e293b"}}>{s.name}</span>}
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:10,color:"#94a3b8"}}>{s.weight}%</span>
                  <span style={{fontFamily:"'JetBrains Mono'",fontSize:14,fontWeight:800,color:s.score>=9.5?"#059669":"#0d9488"}}>{s.score}</span>
                </div>
              </div>
              ...
              <div style={{fontSize:13,color:"#64748b",lineHeight:1.6}}>{s.detail}</div>
            </div>})}
          ...
          <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:14,marginBottom:16}}>
            <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:12,padding:"20px"}}>
              <div style={{fontFamily:"Outfit",fontWeight:700,fontSize:14,color:"#059669",marginBottom:12}}>{t("review.pros")}</div>
              {pros.map((p,i)=><div key={i} style={{fontSize:13,color:"#1e293b",marginBottom:8,paddingLeft:16,position:"relative",lineHeight:1.5}}><span style={{position:"absolute",left:0,color:"#059669"}}>{"\u2022"}</span>{p}</div>)}
            </div>
            <div style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:12,padding:"20px"}}>
              <div style={{fontFamily:"Outfit",fontWeight:700,fontSize:14,color:"#dc2626",marginBottom:12}}>{t("review.cons")}</div>
              {cons.map((c,i)=><div key={i} style={{fontSize:13,color:"#1e293b",marginBottom:8,paddingLeft:16,position:"relative",lineHeight:1.5}}><span style={{position:"absolute",left:0,color:"#dc2626"}}>{"\u2022"}</span>{c}</div>)}
            </div>
          </div>
          ...
          <Card style={{padding:0,overflow:"hidden"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr style={{background:"#f8f9fb",borderBottom:"1px solid #e8ecf1"}}>
                {[t("table.account"),t("table.spreadFrom"),t("table.commission"),t("table.minDeposit"),t("table.bestFor")].map(h=><th key={h} style={{textAlign:"left",padding:"10px 14px",fontSize:13,fontWeight:600,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.04em"}}>{h}</th>)}
              </tr></thead>
              <tbody>{ACCOUNTS.map((a,i)=><tr key={i} style={{borderBottom:i<ACCOUNTS.length-1?"1px solid #f0f4f8":"none"}}>
                ...
                <td style={{padding:"12px 14px",color:"#64748b",fontSize:12}}>{a.best}</td>
              </tr>)}</tbody>
            </table>
          </Card>
          ...
          <Card style={{padding:0,overflow:"hidden"}}>
            {B.regs.map((r,i)=><div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 18px",borderBottom:i<B.regs.length-1?"1px solid #f0f4f8":"none"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <span style={{background:r.tier===1?"#ecfdf5":"#fffbeb",color:r.tier===1?"#059669":"#d97706",fontSize:10,fontWeight:600,padding:"3px 8px",borderRadius:4,border:`1px solid ${r.tier===1?"#a7f3d0":"#fde68a"}`}}>Tier {r.tier}</span>
                {getRegulatorSlug(r.name) ? <Link to={lp(`/regulator/${getRegulatorSlug(r.name)}`)} style={{fontWeight:600,fontSize:14,color:"#1e293b",textDecoration:"none"}}>{r.name}</Link> : <span style={{fontWeight:600,fontSize:14}}>{r.name}</span>}
                <span style={{fontSize:12,color:"#94a3b8"}}>{r.country}</span>
              </div>
              <span style={{fontFamily:"'JetBrains Mono'",fontSize:12,color:"#64748b"}}>#{r.num}</span>
            </div>)}
          </Card>
          ...
          <Card>
            <div style={{display:"grid",gridTemplateColumns:mob?"1fr 1fr":"1fr 1fr 1fr",gap:16,marginBottom:14}}>
              {costBoxes.map((x,i)=><div key={i} style={{textAlign:"center"}}>
                <div style={{fontSize:12,color:"#94a3b8",marginBottom:4}}>{x.l}</div>
                <div style={{fontFamily:"'JetBrains Mono'",fontSize:22,fontWeight:800,color:"#059669"}}>{x.v}</div>
                <div style={{fontSize:12,color:"#94a3b8"}}>{x.n}</div>
              </div>)}
            </div>
          </Card>
          ...
          <Card style={{padding:0,overflow:"hidden"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <thead><tr style={{background:"#f8f9fb",borderBottom:"1px solid #e8ecf1"}}>
                <th style={{textAlign:"left",padding:"9px 12px",fontSize:12,fontWeight:600,color:"#94a3b8",textTransform:"uppercase"}}>{t("table.pair")}</th>
                {spreadCompetitors.map(h=><th key={h} style={{textAlign:"left",padding:"9px 12px",fontSize:12,fontWeight:600,color:"#94a3b8",textTransform:"uppercase"}}>{h}</th>)}
              </tr></thead>
          ...
          <Card style={{padding:0,overflow:"hidden"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr style={{background:"#f8f9fb",borderBottom:"1px solid #e8ecf1"}}>
                {[t("table.method"),t("table.fee"),t("table.minimum"),t("table.processingTime")].map(h=><th key={h} style={{textAlign:"left",padding:"9px 14px",fontSize:12,fontWeight:600,color:"#94a3b8",textTransform:"uppercase"}}>{h}</th>)}
              </tr></thead>
          ...
          <Card style={{display:"flex",alignItems:mob?"flex-start":"center",gap:mob?16:24,flexDirection:mob?"column":"row"}}>
            <a href={getTrustpilotUrl(slug)} target="_blank" rel="noopener noreferrer" style={{textAlign:"center",flexShrink:0,textDecoration:"none",color:"inherit"}}>
              ...
              <div style={{fontSize:12,color:"#94a3b8",marginTop:4}}>{B.tpCount.toLocaleString()} {t("review.reviews")}</div>
            </a>
            ...
            <div style={{flex:1,width:mob?"100%":"auto"}}>
              {trustpilotBars.map((x,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                <span style={{fontSize:11,color:"#94a3b8",width:24}}>{x.s}</span>
                ...
                <span style={{fontSize:11,color:"#64748b",width:30,textAlign:"right"}}>{x.p}%</span>
              </div>)}
            </div>
          </Card>
          <a href={getTrustpilotUrl(slug)} target="_blank" rel="noopener noreferrer" style={{display:"inline-flex",alignItems:"center",gap:6,fontSize:13,fontWeight:600,color:"#00B67A",textDecoration:"none",marginBottom:12}}>
          ...
          <div style={{position:"relative",paddingLeft:24,marginBottom:16}}>
            ...
            {TIMELINE.map((tl,i)=><div key={i} style={{position:"relative",marginBottom:14,paddingLeft:16}}>
              ...
              <span style={{fontFamily:"'JetBrains Mono'",fontSize:13,fontWeight:700,color:"#1e3a5f"}}>{tl.year}</span>
              <span style={{fontSize:13,color:"#475569",marginLeft:8}}>{tl.event}</span>
            </div>)}
          </div>
          ...
          {SIMILAR.map((b,i)=>{const altData=getBrokerData(b.slug);const altUrl=altData?.B?.url||`https://ratedbrokers.com/go/${b.slug}`;return <Card key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div>
              ...
              >{b.name}</Link>
              <div style={{fontSize:13,color:"#64748b"}}>{b.type} {"\u00b7"} {t("home.from")} {b.spread} {t("home.pips")} {"\u00b7"} {b.why}</div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          {faq.map((f,i)=><div key={i} style={{background:"#fff",border:"1px solid #e8ecf1",borderRadius:10,marginBottom:6,overflow:"hidden"}}>
            ...
            {openFaq===i&&<div style={{padding:"0 18px 14px",fontSize:15,color:"#64748b",lineHeight:1.7}}>{f.a}</div>}
          </div>)}
        ...
        {!mob&&<aside>
          <div style={{position:"sticky",top:70,display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Card style={{padding:"16px"}}>
              <div style={{fontFamily:"Outfit",fontWeight:700,fontSize:12,marginBottom:10}}>{t("review.quickFacts")}</div>
              {[{l:t("review.founded"),v:B.year},{l:t("review.hq"),v:B.hq},{l:t("review.deposit"),v:`$${B.minDep}`},{l:t("review.spread"),v:`${B.spread} pips`},{l:t("review.leverage"),v:B.leverage},{l:t("review.type"),v:B.type},{l:t("review.instruments"),v:B.instruments}].map((x,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:i<6?"1px solid #f0f4f8":"none"}}>
                ...
                <span style={{fontSize:12,color:"#1e293b",fontWeight:600}}>{x.v}</span>
              </div>)}
            </Card>
            <Card style={{padding:"16px"}}>
