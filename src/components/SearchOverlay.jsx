...
export default function SearchOverlay({ onClose }) {
  ...
  const footer = {
    ...
    fontSize: 13, color: "#1f2937", flexShrink: 0,
  };
  ...
  const kbd = {
    ...
    fontFamily: "'JetBrains Mono', monospace", color: "#1f2937",
  ...
  const renderMeta = (item) => {
    ...
    if (item.type === "country" && item.meta.code) {
  ...
  return createPortal(
    <div style={backdrop} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={modal} onKeyDown={handleKey}>
        {/* ── Input ── */}
        <div style={inputWrap}>
          <Search size={20} color="#1f2937" style={{ flexShrink: 0 }} />
          <input
            ref={inputRef}
            type="text"
            value={query}
          ...
          <button onClick={onClose} style={{
            ...
            padding: 4, display: "inline-flex", color: "#1f2937",
          }}><X size={20} /></button>
        </div>
        ...
        <div ref={listRef} style={{ flex: 1, overflowY: "auto", overscrollBehavior: "contain" }}>
          ...
          {query.length > 0 && query.length < 2 && (
            <div style={{ padding: "40px 20px", textAlign: "center", color: "#1f2937", fontSize: 15 }}>
              {t("search.minChars")}
            </div>
          )}
          ...
          {query.length >= 2 && results.length === 0 && (
            <div style={{ padding: "40px 20px", textAlign: "center" }}>
              <div style={{ fontSize: 15, color: "#1f2937", fontWeight: 500, marginBottom: 6 }}>
