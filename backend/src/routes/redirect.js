export async function handleRedirect(request, env, slug) {
  ...
  const sourcePage = url.searchParams.get('ref') || null;
  ...
  const trackClick = env.DB.prepare(
    'INSERT INTO clicks (broker_slug, referrer, country, user_agent, source_page) VALUES (?, ?, ?, ?, ?)'
  ).bind(slug, referrer, country, userAgent, sourcePage).run();
